...
int multidnslookup(const char* hostname, char* returnIPstr, int maxSize){
	...
	if(result->ai_addr->sa_family == AF_INET){
	    ...
	    strncpy(ipstr, ipv4str, sizeof(ipstr));
	    ipstr[sizeof(ipstr)-1] = '\0';
		if(strstr(returnIPstr, ipstr) == NULL)
		{
			strcat(returnIPstr, ipstr);
			strcat(returnIPstr, ", ");
		}
	...
	/*if(result==headresult){
		...
		while(result != NULL)
		{
	    	//strncpy(returnIPstr, ipstr, maxSize);
			strcat(returnIPstr, ipstr);
			strcat(returnIPstr, ", ");
	    	//returnIPstr[maxSize-1] = '\0';
